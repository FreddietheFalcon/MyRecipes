...
export async function getAllIngredients(req, res) {
  try {
    const ingredients = await Ingredient.find({ userId: req.user.id }).sort({ name: 1 });
...
export async function getIngredientById(req, res) {
  try {
    const ingredient = await Ingredient.findOne({ _id: req.params.id, userId: req.user.id });
...
export async function createIngredient(req, res) {
  try {
    ...
    const ingredient = new Ingredient({ userId: req.user.id, name, amount, unit, status });
...
export async function deleteIngredient(req, res) {
  try {
    const deleted = await Ingredient.findOneAndDelete({ _id: req.params.id, userId: req.user.id });
